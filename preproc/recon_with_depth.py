...
def load_calib_from_colmap(sparse_dir: str, img_dir: str) -> dict[str, list[float]]:
    from pycolmap import SceneManager
    manager = SceneManager(sparse_dir, image_path=img_dir)
    manager.load()

    calib_dict = {}
    for image_id, image in manager.images.items():
        K = manager.cameras[image.camera_id].get_camera_matrix()
        fx, fy = K[0, 0], K[1, 1]
        cx, cy = K[0, 2], K[1, 2]
