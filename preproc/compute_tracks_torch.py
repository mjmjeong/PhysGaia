...
def read_video(folder_path, matching_pattern=""):
    all_frame_paths = sorted(glob.glob(os.path.join(folder_path, "*")))
    frame_paths = [f for f in all_frame_paths if fnmatch.fnmatch(os.path.basename(f), matching_pattern)]
...
def main():
    ...
    parser.add_argument("--matching_pattern", type=str, default="0_*", help="pattern for matching files")
    ...
    video = read_video(folder_path, args.matching_pattern)
    num_frames, height, width = video.shape[0:3]
    masks = read_video(mask_dir, args.matching_pattern)
