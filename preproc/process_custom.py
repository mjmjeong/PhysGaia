...
#             # Get base directory: remove trailing img_name and swap PhysTrack -> PhysTrack_SOM
#             assert img_name in img_dir, f"Expecting {img_name} in path: {img_dir}"
#             base_dir = str(img_path).replace(f"/{img_name}", "").replace("PhysTrack", "PhysTrack_SOM")

#             exc.submit(
#                 process_sequence,
#                 gpu,
#                 img_dir,
#                 f"{base_dir}/{mask_name}",
#                 f"{base_dir}/{metric_depth_name}",
#                 f"{base_dir}/{intrins_name}.json",
#                 f"{base_dir}/{mono_depth_name}",
#                 f"{base_dir}/aligned_{mono_depth_name}",
#                 f"{base_dir}/{slam_name}",
#                 f"{base_dir}/{track_model}",
#                 mono_depth_model,
#                 track_model,
#                 tapir_torch,
#             )
