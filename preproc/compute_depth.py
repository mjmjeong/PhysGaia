...
def align_monodepth_with_colmap(
    ...
    if camera_json_path:
        K_dict, extrinsics_dict = load_intrinsics_and_extrinsics_from_camera_json(camera_json_path, img_dir)
    ...
    for image in tqdm(images, "Aligning monodepth with colmap point cloud"):
        ...
        if pts2d_valid_cam.size == 0:
            print(f"[Warning] pts2d_valid_cam is empty for image {image.name}")
            continue
...
def load_intrinsics_and_extrinsics_from_colmap(sparse_dir: str, matching_pattern: str = "*"):
    from pycolmap import SceneManager
    import fnmatch
    ...
    for image_id, image in manager.images.items():
        if not fnmatch.fnmatch(image.name, matching_pattern):
            continue
...
def align_monodepth_with_colmap_ply(
    ...
    os.makedirs(output_monodepth_dir, exist_ok=True)
    K_dict, extrinsics_dict = load_intrinsics_and_extrinsics_from_colmap(sparse_dir, matching_pattern)
    if camera_json_path:
        K_dict, extrinsics_dict = load_intrinsics_and_extrinsics_from_camera_json(camera_json_path, img_dir)
    
    img_files = sorted([f for f in os.listdir(img_dir) if fnmatch.fnmatch(f, matching_pattern)])
    for image_name in tqdm(img_files, desc="Aligning monodepth with PLY point cloud"):
        ...
        points_h = np.concatenate([points, np.ones_like(points[:, :1])], axis=1)  # [N, 4]
        cam_points = (T_w2c @ points_h.T).T  # [N, 4]
        z_vals = cam_points[:, 2]
        valid = z_vals > 1e-6
        ...
        pts2d = pts2d[:, :2] / pts2d[:, 2:3]  # [N, 2]
        colmap_disp = 1.0 / np.clip(cam_points[:, 2], a_min=1e-6, a_max=1e6)

        # Sample monodepth at 2D locations
        ...
        # Use scipy to sample at sparse locations
        coords = np.stack([pts2d[:, 1], pts2d[:, 0]], axis=0)  # (2, N), [y, x]
        sampled_mono_disp = map_coordinates(mono_disp_map, coords, order=1, mode='constant', cval=0.0)

        valid_mask = (sampled_mono_disp > 0) & (colmap_disp > 0)

        ms_colmap_disp = colmap_disp[valid_mask] - np.median(colmap_disp[valid_mask]) + 1e-8
