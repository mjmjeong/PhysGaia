...
if __name__ == "__main__":
    ...
    render_args = [
        "--configpath", "configs/base_phystrack_full.json"
    ]
    ...
    for i, (source_path, init_option, num_view) in enumerate(all_experiments):
        ...
        # Check if 30000_runtimeresults.json file exists
        metric_json_path = os.path.join(save_path, "30000_runtimeresults.json")
        if os.path.exists(metric_json_path):
            logger.info(f"'30000_runtimeresults.json' file already exists at {metric_json_path}, skipping experiment {expname}")
            results["skipped"].append({
                "index": i,
                "experiment": expname,
