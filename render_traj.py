...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        render_dict = render(view, gaussians, pipeline, background,cam_type=cam_type)
        means3D = render_dict["means3D"]

        particles = means3D.reshape(-1, 3).cpu().numpy()
        particle_list = []
        for idx, particle in enumerate(particles):
            particle_list.append({"id": idx, "position": particle.tolist()})
        particles_dir = os.path.join(model_path, "./particles/4dgs/")
        makedirs(particles_dir, exist_ok=True)
        particles_file = os.path.join(particles_dir, f"particles_frame_{int(view.image_name)+1:04d}.json")
        with open(particles_file, "w") as f:
            import json
            json.dump(particle_list, f)
